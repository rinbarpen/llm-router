...
@pytest.fixture
async def app_client(tmp_path: Path, stub_registry):
    """创建测试应用和客户端"""
...
async def app_client_with_auth(tmp_path: Path, stub_registry):
    ...
    async with LifespanManager(app):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as client:
            yield client
    load_settings.cache_clear()
    for key in ("LLM_ROUTER_DATABASE_URL", "LLM_ROUTER_MODEL_STORE", "LLM_ROUTER_REQUIRE_AUTH", "LLM_ROUTER_API_KEYS", "LLM_ROUTER_ALLOW_LOCAL_WITHOUT_AUTH"):
...
async def test_protected_endpoint_returns_403_with_invalid_key(app_client_with_auth: AsyncClient) -> None:
    response = await app_client_with_auth.get("/models", headers={"Authorization": "Bearer invalid-key"})
    assert response.status_code == 403


@pytest.mark.asyncio
async def test_protected_endpoint_returns_200_with_valid_api_key(app_client_with_auth: AsyncClient) -> None:
    response = await app_client_with_auth.get("/models", headers={"Authorization": "Bearer auth-fixture-key"})
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_protected_endpoint_returns_200_with_x_api_key(app_client_with_auth: AsyncClient) -> None:
    response = await app_client_with_auth.get("/models", headers={"X-API-Key": "auth-fixture-key"})
    assert response.status_code == 200
...
async def test_protected_endpoint_returns_200_with_session_token(app_client_with_auth: AsyncClient) -> None:
    ...
    response = await app_client_with_auth.get("/models", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
